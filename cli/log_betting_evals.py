...
def deduplicate_pending_quiet_logs(path: str = PENDING_QUIET_LOGS_PATH) -> int:
    """Remove duplicate queued bets keeping the latest entry for each key.
    ...
    seen = set()
    deduped_latest_first: list[dict] = []
    for row in reversed(queue):
        key = f"{row.get('game_id')}:{row.get('market')}:{row.get('side')}"
        if key not in seen:
            deduped_latest_first.append(row)
            seen.add(key)

    deduped = list(reversed(deduped_latest_first))
    if len(deduped) != len(queue):
        save_quiet_log_queue(deduped, path)
    return len(queue) - len(deduped)
...
def process_quiet_hour_queue(market_odds: dict, min_ev: float = 0.05) -> int:
    ...
    # 🔄 Remove stale duplicates before processing
